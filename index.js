...
function makeClientFromData(data) {
  ...
  if (!client.name)
    errors.push({ field: 'name', message: 'Name has not been provided' })
  if (!client.surname)
    errors.push({ field: 'surname', message: 'Surname has not been provided' })
  if (client.contacts.includes((contact) => !contact.type || !contact.value))
    errors.push({
      field: 'contacts',
      message: 'Not all contacts are fully filled',
    })
...
module.exports = createServer(async (req, res) => {
  ...
  .on('listening', () => {
    if (process.env.NODE_ENV !== 'test') {
      console.log(
        `The CRM server is running. You can use it at http://localhost:${PORT}`
      )
      console.log('Press CTRL+C to stop server')
      console.log('Available methods:')
      console.log(
        `GET ${URI_PREFIX} - get clients list, you can submit your search phrase in parameter "search"`
      )
      console.log(
        `POST ${URI_PREFIX} - create the client, in the request body you must send the object: { name: string, surname: string, lastName?: string, contacts?: object[] }`
      )
      console.log(
        `\tcontacts - array of objects: { type: string, value: string }`
      )
      console.log(`GET ${URI_PREFIX}/{id} - get client by ID`)
      console.log(
        `PATCH ${URI_PREFIX}/{id} - edit client with ID, in the request body you must send the object: { name?: string, surname?: string, lastName?: string, contacts?: object[] }`
      )
      console.log(
        `\tcontacts - array of objects: { type: string, value: string }`
      )
